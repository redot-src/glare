...
import FramedPage from './demo/FramedPage.vue'
...
const theme: Theme = {
  ...
  enhanceApp({ app }) {
    ...
    // The `iframe` example delegates to this window and looks for Glare on it, as a CDN build would expose it.
    if (!import.meta.env.SSR) window.Glare = Glare

    app.component('DemoHero', DemoHero)
    app.component('ContentTypes', ContentTypes)
    app.component('OptionRecipes', OptionRecipes)
    app.component('FramedPage', FramedPage)
  },
}
